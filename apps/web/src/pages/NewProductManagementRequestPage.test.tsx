...
const emptyApi = {
  ...
  form: async (country: string, topic: string) => ({ source: "MOCK" as const, country, topic, schema: { legacyScreenPattern: "Synthetic", legacyFormPattern: "Synthetic", lookupRequirements: [], implementationStatus: "PARTIAL" as const, partialReasons: ["UNKNOWN_SUBMISSION_MAPPING" as const], submissionEnabled: false }, fields: [] }),
...
test("New Request renders five Countries, common Topics, partial schema state and Account dependent Role", async () => {
  ...
  const api = {
    ...
    form: async (country: string, topic: string) => ({
      ...
      country,
      topic,
      schema: { legacyScreenPattern: "TH base; other {CC}_เพิ่ม Permission เข้า Role(ลูกค้า)_{CC}", legacyFormPattern: "Form* (DataSource: USR_PowerApp)", lookupRequirements: ["account", "customerRole"] as const, implementationStatus: "PARTIAL" as const, partialReasons: ["UNKNOWN_SUBMISSION_MAPPING" as const], submissionEnabled: false },
      fields: [
