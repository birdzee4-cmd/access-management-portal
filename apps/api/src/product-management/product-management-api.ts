...
function parseSubmission(value: unknown): ProductManagementRequestSubmission | null {
  ...
  if (form.schema.implementationStatus !== "CONFIRMED" || !form.schema.submissionEnabled) return null;
...
export async function handleProductManagementLookup(request: ProductManagementHttpRequest, dependencies: ProductManagementApiDependencies): Promise<HttpResponseInit> { try { await identity(request, dependencies); const get = (name: string) => request.query?.get(name)?.trim() || undefined; const country = get("country"), topic = get("topic"); if (!country || !topic) throw new ProductManagementMasterDataValidationError(); return result(200, await dependencies.getMasterDataService().lookup(request.params?.lookup ?? "", { country, topic, account: get("account") })); } catch (error) { return errorResponse(error); } }
