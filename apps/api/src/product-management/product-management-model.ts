...
export const productManagementSchemaRegistry: readonly ProductManagementSchemaRegistryEntry[] = [
  schema(productManagementTopics[0], "{CC}_สร้างAccountลูกค้า", [
    ...
    lookup("packageAddOn", "Package Add On", "packageAddOn", "CONFIRMED_OPTIONAL", "MULTIPLE", { legacyDataField: "PackageHid(Product)", legacyLabel: "Package Add On", legacyControl: "Classic/ComboBox", legacyBinding: "SelectedItems", legacyDefault: "blank", legacyVisibility: "VISIBLE", legacyLookupSource: "country-partitioned DB - vw_ListPackagHidden aliases", portalHandling: "ALLOW_EDIT", submitDestination: "USR_PowerApp.PackageHid(Product) only; intentionally omitted from Detail, Product Management SQL, and VSTS", transformation: "Concat DisplayName values with the exact ' , ' delimiter; no downstream transformation", serialization: selectedItems("Concat(PackageAddOn.SelectedItems, DisplayName, \" , \")", " , ", "USR_PowerApp.PackageHid(Product) only", "No Product Management SQL parameter, Detail fragment, approval payload, or VSTS field consumes this value"), ownerDecision: ownerDecision(["PMD-001"], "APPROVE_RECOMMENDATION", "Typed multi-value Package Add-On concept", "Preserve the technically verified SharePoint-only legacy mapping; do not invent a SQL, Detail, or VSTS destination", ["Preserve multiple values", "Serialize only to PackageHid(Product) at the legacy boundary", "Omit from Detail, SQL, and VSTS compatibility output"]) }),
  ], []),
  ...
  schema(productManagementTopics[9], "{CC}_เปลี่ยน Provider สำหรับ Account(ลูกค้า)", [
    lookup("account", "Account", "account", "CONFIRMED_REQUIRED", "SINGLE", { legacyDataField: "AccountName(Product)", legacyLabel: "Account Name", legacyControl: "Classic/ComboBox", legacyBinding: "Selected country account display column", legacyDefault: "blank", legacyVisibility: "VISIBLE", legacyLookupSource: "country-partitioned DB - AccountName_EX aliases", portalHandling: "ALLOW_EDIT", submitDestination: "USR_PowerApp.AccountName(Product) -> SQL.AccountName; also encoded in Detail" }),
    lookup("providerType", "New Provider Type", "providerType", "CONFIRMED_REQUIRED", "SINGLE", { legacyDataField: "RoleName(Product)", legacyLabel: "Name Provider Type (New)", legacyControl: "Classic/DropDown", legacyBinding: "Selected.Value", legacyDefault: "Select Providers Type sentinel", legacyVisibility: "VISIBLE", legacyLookupSource: "hard-coded Provider Type choices", portalHandling: "ALLOW_EDIT", submitDestination: "USR_PowerApp.RoleName(Product) -> SQL.RoleName; also encoded in Detail -> VSTS description", transformation: "Owner-approved Phase 1 Legacy compatibility: New Provider reuses RoleName(Product); the Flow copies it unchanged to SQL.RoleName and carries the same provider meaning through Detail", legacyFieldReuse: reuse("New Provider Type", "RoleName(Product)", "dbo.UserRequest_ProductManagement.RoleName", "PMD-011") }),
  ...
  schema(productManagementTopics[10], "{CC}_Tranfer Owner Account(ลูกค้า)", [
    lookup("account", "Account", "account", "CONFIRMED_REQUIRED", "SINGLE", { legacyDataField: "AccountName(Product)", legacyLabel: "Account Name", legacyControl: "Classic/ComboBox", legacyBinding: "Selected country account display column", legacyDefault: "blank", legacyVisibility: "VISIBLE", legacyLookupSource: "country-partitioned DB - AccountName_EX aliases", portalHandling: "ALLOW_EDIT", submitDestination: "USR_PowerApp.AccountName(Product) -> SQL.AccountName; also encoded in Detail" }),
    lookup("providerType", "New Provider Type", "providerType", "CONFIRMED_REQUIRED", "SINGLE", { legacyDataField: "RoleName(Product)", legacyLabel: "Name Provider Type (New)", legacyControl: "Classic/DropDown", legacyBinding: "Selected.Value", legacyDefault: "Select Providers Type sentinel", legacyVisibility: "VISIBLE", legacyLookupSource: "hard-coded Provider Type choices", portalHandling: "ALLOW_EDIT", submitDestination: "USR_PowerApp.RoleName(Product) -> SQL.RoleName; also encoded in Detail -> VSTS description", transformation: "Owner-approved Phase 1 Legacy compatibility: Transfer Owner Provider reuses RoleName(Product); the Flow copies it unchanged to SQL.RoleName and carries the same provider meaning through Detail", legacyFieldReuse: reuse("New Provider Type selected during Transfer Owner", "RoleName(Product)", "dbo.UserRequest_ProductManagement.RoleName", "PMD-012") }),
    text("customerEmail", "Customer Email", "CONFIRMED_REQUIRED", "SINGLE", { legacyDataField: "Detail", legacyLabel: "Email ลูกค้า", legacyControl: "Classic/TextInput", legacyBinding: "Text", legacyDefault: "blank", legacyVisibility: "VISIBLE", portalHandling: "ALLOW_EDIT", submitDestination: "encoded only in USR_PowerApp.Detail -> SQL.Detail -> VSTS description; not mapped to SQL.Email_Customer", transformation: "Raw text is appended unchanged after the Email label; retains the owner-approved Legacy Customer Email semantics and establishes no identity or ownership authority", serialization: rawText("Detail = Topic + Account + NewProvider + \"Email : \" + TextInput.Text", "USR_PowerApp.Detail -> SQL.Detail", "Manager approval and VSTS description through Detail; no dedicated Transfer Owner email field"), ownerDecision: ownerDecision(["PMD-013"], "APPROVE_WITH_CHANGE_LEGACY_COMPATIBILITY_FIRST", "Customer Email compatibility value", "Retain Email ลูกค้า / Customer Email and never infer Account ownership or identity authority", ["Required value", "No authority derived from the address", "Do not rename to newOwnerEmail"]) }),
  ], []),
